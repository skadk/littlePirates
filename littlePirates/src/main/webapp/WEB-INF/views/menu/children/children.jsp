				...
				<div class="middle">
					...
					<table border="1" class="table">
						<c:forEach var="kd" items="${KList }">
							<tr>
								<td><img src="<c:url value='/images/${kd.kdName}.png' />" width="180" height="120"></td>
								<td>${kd.kdName }<br>
									${kd.kdAddress}<br>
									${kd.kdType}<br>
									${kd.kdTel}</td>
					...
					<table border="1" class="table2">
						<c:forEach var="nur" items="${NList }">
							<tr>
								<td><img src="<c:url value='/images/${nur.nurName}.jpg' />" width="180" height="120"></td>
								<td>${nur.nurName }<br>
									${nur.nurAddress}<br>
									${nur.nurType}<br>
									${nur.nurTel}</td>
				...
				<div>
					<div style="display: block; text-align: center;">
						<c:if test="${paging.startPage != 1 }">
							<a href="/children?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
						</c:if>
						<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
							<c:choose>
								...
								<c:when test="${p != paging.nowPage }">
									<a href="/children?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
								</c:when>
							</c:choose>
						</c:forEach>
						<c:if test="${paging.endPage != paging.lastPage}">
							<a href="/children?nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}">&gt;</a>
						</c:if><br>
					</div>
				</div>
			</section>
